      ...
      <div class="popup__inner">
        <aside class="side">
          ...
          <div class="side__main">
            ...
            <div class="side__description">
              <p v-if="show_edit_text" class="description__text" @click="showEditDescForm()">
                {{image_data.description}}
              </p>


              <form v-if="show_edit_form" class="description__form" @submit.prevent="editImageDescription">
                <textarea class="description__textarea" v-model="image_desc" placeholder="Описание"></textarea>
                <button class="description__btn">редактировать</button>
              </form>

              <p class="description__edit" @click="showEditDescForm()">Описание добавить/сменить</p>
            </div>
        ...
        <main class="main">
          ...
          <div class="popup__footer">
            <button class="popup__footer-delete" @click="deleteImg">Удалить</button>
          </div>
        </main>
      </div>
    </div>
  ...
  methods: {
    ...
    showEditDescForm() {
      ...
      if (this.show_edit_form) {
        let el = document.querySelector('.description__text')
    ...
    editImageDescription() {
      ...
      formData.append('image_id', this.image.id)
      formData.append('description', this.image_desc.trim())
      formData.append('_method', 'PUT')

      this.editImageDesc(formData).then(resp => {
          if (resp.status === 200) {
            this.show_edit_text = true
            this.show_edit_form = false

            this.image_data = resp.data
          }
      })
    },
    ...
    deleteImg() {
      this.deleteImage(this.image.id).then(resp => {
        if (resp === 200) {
          this.$emit('closePopup')
        }
      })
