      ...
      <div class="container">
        <div class="users__inner">
          ...
          <main class="users">
            ...
            <h2 class="users__title">Ваши друзья</h2>

            <!--//Принять дружбу-->
            <friend-accept
                v-if="getAcceptFriends.length"
                :friends = "getAcceptFriends"
                @acceptFriendship = "acceptFriend"
                @rejectFriend  = "deleteFromFriends"
            />
            ...
            <div v-else class="user" v-for="user in getFriends">
              ...
              <div class="user-info__action">
                <button class="user-info__btn" @click="deleteFromFriends(user.id)">
                   {{user.relation_type ? 'Убрать' : 'В подписчики'}}
                </button>
...
export default {
  ...
  components: {LeftSecondBar, FriendAccept, PopupMessage, RightBar, LeftBar},
  methods: {
    ...mapActions([
      'fetchFriends',
    ...
    acceptFriend(friend_id) {
      ...
      this.acceptFriendInBd({user_id: this.current_user_id, friend_id})
  ...
  mounted() {
    ...
    this.receiveCurrentUser(userId)
  }
}
</script>
